...
float get_cost(string item)
{
    ...
    for (int i = 0; i < NUM_ITEMS; i++)
    {
        char *lowerMenuItem = malloc(strlen(menu[i].item) + 1);

        if (lowerMenuItem == NULL)
        {
            return 0.0;
        }

        strcpy(lowerMenuItem,menu[i].item);

        for (int k = 0; lowerMenuItem[k] != '\0'; k++)
        {
            lowerMenuItem[k] = tolower(lowerMenuItem[k]);
        }

        if (strcmp(lowerItem, lowerMenuItem) == 0)
        {
            free(lowerMenuItem);
            return menu[i].price;
        }
